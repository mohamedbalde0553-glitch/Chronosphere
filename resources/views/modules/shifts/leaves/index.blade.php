<x-app-layout>
    ...
    <div x-data="leavesApp()" @keydown.escape.window="closeModal()">
        ...
        <div class="flex items-center justify-between mb-5">
            <div>
                <h2 class="text-lg font-bold text-gray-900">Demandes de congés</h2>
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-3">
                            <div class="font-medium text-gray-900">{{ $leave->employee->user->name }}</div>
                        </td>
                        <td class="px-4 py-3 text-gray-700">{{ $typeLabels[$leave->type] ?? $leave->type }}</td>
                        <td class="px-4 py-3 text-center text-gray-700">{{ $leave->start_date->format('d/m/Y') }}</td>
                        <td class="px-4 py-3 text-center text-gray-700">{{ $leave->end_date->format('d/m/Y') }}</td>
                        <td class="px-4 py-3 text-gray-500 truncate max-w-xs">{{ $leave->reason ?? '—' }}</td>
                        ...
                        <td class="px-4 py-3 text-right whitespace-nowrap">
                            ...
                            <button @click="openEdit({{ $leave }})"
                                    class="text-xs text-blue-600 hover:text-blue-700 font-medium mr-2">Modifier</button>
                            <button @click="deleteItem({{ $leave->id }})"
                                    class="text-xs text-red-500 hover:text-red-600 font-medium">Supprimer</button>
                        </td>
                    </tr>
                    @empty
                    <tr><td colspan="7" class="px-4 py-8 text-center text-gray-400">Aucune demande de congé.</td></tr>
                    @endforelse
                </tbody>
            </table>
            @if($leaves->hasPages())
        ...
        <div x-show="showRejectModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-sm z-10 p-6 space-y-4" @click.stop>
                <h3 class="text-base font-semibold text-gray-900">Rejeter la demande</h3>
                <div>
                ...
                <div class="flex justify-end gap-3">
                    ...
                    <button @click="confirmReject()" :disabled="loading"
                        ...
                        <span x-show="!loading">Confirmer le rejet</span><span x-show="loading">…</span>
                    </button>
                </div>
            </div>
        </div>
            ...
            return {
                ...
                async save() {
                    ...
                    const url    = this.editMode
                        ? leaveUrls.updateBase.replace('__ID__', this.itemId)
                        : leaveUrls.store;
                ...
                async deleteItem(id) {
                    ...
                    try {
                        const res = await fetch(leaveUrls.deleteBase.replace('__ID__', id), {
                ...
                async approve(id) {
                    ...
                    this.loading = true;
                    try {
                        const res = await fetch(leaveUrls.approveBase.replace('__ID__', id), {
                ...
                async confirmReject() {
                    this.loading = true;
                    try {
                        const res = await fetch(leaveUrls.rejectBase.replace('__ID__', this.rejectId), {
                            method: 'POST',
                            headers: { 'Content-Type':'application/json','X-CSRF-TOKEN': csrf },
