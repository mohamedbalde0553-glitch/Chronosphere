<x-app-layout>
    ...
    <div x-data="crudTable({
        ...
        <div class="flex items-center justify-between mb-5">
            <div>
                <h2 class="text-lg font-bold text-gray-900 dark:text-white">Départements</h2>
                <p class="text-sm text-gray-500 dark:text-gray-400">{{ $departments->total() }} département(s)</p>
            </div>
            ...
            <table class="w-full text-sm min-w-[640px]">
                <thead class="bg-gray-50 dark:bg-gray-700 border-b border-gray-200 dark:border-gray-600">
                    <tr>
                        ...
                        <th class="text-left px-4 py-3 font-semibold text-gray-600 dark:text-gray-400">Responsable</th>
                        <th class="text-left px-4 py-3 font-semibold text-gray-600 dark:text-gray-400">Parent</th>
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                        ...
                        <td class="px-4 py-3 font-mono font-semibold text-blue-700 dark:text-blue-400">{{ $dept->code }}</td>
                        <td class="px-4 py-3 font-medium text-gray-900 dark:text-white">{{ $dept->name }}</td>
                        <td class="px-4 py-3 text-gray-600 dark:text-gray-300">{{ $dept->manager?->user?->name ?? '—' }}</td>
                        <td class="px-4 py-3 text-gray-500 dark:text-gray-400">{{ $dept->parent?->name ?? '—' }}</td>
                        <td class="px-4 py-3 text-gray-500 dark:text-gray-400 truncate max-w-xs">{{ $dept->description ?? '—' }}</td>
                        <td class="px-4 py-3 text-right">
                            <button @click="openEdit({{ $dept }})" class="text-xs text-blue-600 dark:text-blue-400 hover:text-blue-700 font-medium mr-3">Modifier</button>
                            <button @click="deleteItem({{ $dept->id }})" class="text-xs text-red-500 hover:text-red-600 font-medium">Supprimer</button>
                        </td>
                    </tr>
                    @empty
                    <tr><td colspan="7" class="px-4 py-8 text-center text-gray-400 dark:text-gray-500">Aucun département enregistré.</td></tr>
                    @endforelse
                </tbody>
            </table>
            </div>
            @if($departments->hasPages())
            <div class="px-4 py-3 border-t border-gray-100 dark:border-gray-700">{{ $departments->links() }}</div>
            @endif
        </div>
        ...
        <div x-show="showModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4"
            ...
            <div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-md z-10 p-6 space-y-4" @click.stop>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Département parent</label>
