<x-app-layout>
    <x-slot name="title">Catégories de ressources</x-slot>

    <div x-data="crudTable({
        storeUrl:  '{{ route('booking.categories.store') }}',
        updateBase:'{{ url('booking/categories/__ID__') }}',
        deleteBase:'{{ url('booking/categories/__ID__') }}',
        emptyForm: () => ({ name:'', icon:'', color:'#EA580C', description:'' }),
    })">

        <div class="flex items-center justify-between mb-5">
            <div>
                <h2 class="text-lg font-bold text-gray-900 dark:text-white">Catégories de ressources</h2>
                <p class="text-sm text-gray-500 dark:text-gray-400">{{ $categories->total() }} catégorie(s)</p>
            </div>
            ...
            <table class="w-full text-sm">
                <thead class="bg-gray-50 dark:bg-gray-700 border-b border-gray-200 dark:border-gray-600">
                    <tr>
                        ...
                        <th class="text-left px-4 py-3 font-semibold text-gray-600 dark:text-gray-400">Icône</th>
                        <th class="text-center px-4 py-3 font-semibold text-gray-600 dark:text-gray-400">Ressources</th>
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                        <td class="px-4 py-3">
                            <span class="inline-block w-6 h-6 rounded-md border border-gray-200 dark:border-gray-600"
                                  style="background: {{ $cat->color ?? '#EA580C' }}"></span>
                        </td>
                        <td class="px-4 py-3 font-medium text-gray-900 dark:text-white">{{ $cat->name }}</td>
                        <td class="px-4 py-3 text-gray-500 dark:text-gray-400 font-mono text-xs">{{ $cat->icon ?? '—' }}</td>
                        ...
                        <td class="px-4 py-3 text-gray-500 dark:text-gray-400 truncate max-w-xs">{{ $cat->description ?? '—' }}</td>
                        <td class="px-4 py-3 text-right">
                            <button @click="openEdit({{ $cat }})" class="text-xs text-blue-600 dark:text-blue-400 hover:text-blue-700 font-medium mr-3">Modifier</button>
                            <button @click="deleteItem({{ $cat->id }})" class="text-xs text-red-500 dark:text-red-400 hover:text-red-600 font-medium">Supprimer</button>
                        </td>
                    </tr>
                    @empty
                    <tr><td colspan="6" class="px-4 py-8 text-center text-gray-400 dark:text-gray-500">Aucune catégorie enregistrée.</td></tr>
                    @endforelse
                </tbody>
            </table>
            @if($categories->hasPages())
            <div class="px-4 py-3 border-t border-gray-100 dark:border-gray-700">{{ $categories->links() }}</div>
            @endif
        </div>
        ...
        <div x-show="showModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4"
            ...
            <div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-md z-10 p-6 space-y-4" @click.stop>
                ...
                <div>
                    ...
                    <input type="text" x-model="form.name" placeholder="Salles de réunion, Équipements…"
                ...
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Icône (slug)</label>
                        <input type="text" x-model="form.icon" placeholder="building-office, computer…"
                               class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg text-sm font-mono focus:outline-none focus:ring-2 focus:ring-orange-500">
                    </div>
                </div>
