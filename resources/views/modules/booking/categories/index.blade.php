<x-app-layout>
    ...
    <div x-data="crudTable({
        ...
        <div class="flex items-center justify-between mb-5">
            <div>
                <h2 class="text-lg font-bold text-gray-900">Catégories de ressources</h2>
            ...
            <table class="w-full text-sm">
                <thead class="bg-gray-50 border-b border-gray-200">
                    <tr>
                        ...
                        <th class="text-left px-4 py-3 font-semibold text-gray-600">Icône</th>
                        <th class="text-center px-4 py-3 font-semibold text-gray-600">Ressources</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3 text-gray-500 font-mono text-xs">{{ $cat->icon ?? '—' }}</td>
                        <td class="px-4 py-3 text-center">
                            <span class="px-2 py-0.5 rounded-full text-xs font-medium bg-blue-50 text-blue-700">
                                {{ $cat->resources_count }}
                            </span>
                        </td>
                        <td class="px-4 py-3 text-gray-500 truncate max-w-xs">{{ $cat->description ?? '—' }}</td>
                        <td class="px-4 py-3 text-right">
                            <button @click="openEdit({{ $cat }})" class="text-xs text-blue-600 hover:text-blue-700 font-medium mr-3">Modifier</button>
                            <button @click="deleteItem({{ $cat->id }})" class="text-xs text-red-500 hover:text-red-600 font-medium">Supprimer</button>
                        </td>
                    </tr>
                    @empty
                    <tr><td colspan="6" class="px-4 py-8 text-center text-gray-400">Aucune catégorie enregistrée.</td></tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div x-show="showModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-md z-10 p-6 space-y-4" @click.stop>
                ...
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Icône (slug)</label>
                        <input type="text" x-model="form.icon" placeholder="building-office, computer…"
                               class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm font-mono focus:outline-none focus:ring-2 focus:ring-orange-500">
                    </div>
                </div>
