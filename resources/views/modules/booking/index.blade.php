    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
            ...
            <nav class="space-y-1">
                ...
                <a href="{{ route($nav['route']) }}"
                   class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-gray-600 hover:bg-gray-50 hover:text-gray-900 transition-colors
                          {{ request()->routeIs($nav['route']) ? 'bg-orange-50 text-orange-700 font-medium' : '' }}">
            ...
            <div class="mt-5 pt-4 border-t border-gray-100">
                <h4 class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Ressources</h4>
                <div class="space-y-1.5">
                    ...
                    <a href="{{ route('booking.calendar', ['by'=>'resource','id'=>$r->id]) }}"
                       class="flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-gray-50 transition-colors">
                        ...
                        <span class="text-sm text-gray-700 truncate">{{ $r->name }}</span>
                        <span class="ml-auto text-xs text-gray-400">{{ $r->capacity }} pl.</span>
                    </a>
                    @endforeach
                </div>
            </div>
        ...
        <div class="lg:col-span-2 space-y-5">
            ...
            <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="text-sm font-semibold text-gray-700">Prochaines réservations (7 jours)</h3>
                    <a href="{{ route('booking.calendar') }}" class="text-xs text-orange-600 hover:text-orange-700 font-medium">Voir calendrier →</a>
                </div>
                @if($upcomingBookings->isEmpty())
                <p class="text-sm text-gray-400 text-center py-6">Aucune réservation à venir.</p>
                @else
                <div class="space-y-2">
                    @foreach($upcomingBookings as $b)
                    @php $statusBg = $b->status === 'confirmed' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'; @endphp
                    <div class="flex items-center gap-3 p-3 rounded-lg bg-gray-50">
                        ...
                        <div class="flex-1 min-w-0">
                            <p class="text-sm font-medium text-gray-900 truncate">
                                {{ $b->title }}
                                <span class="font-normal text-gray-500">— {{ $b->resource->name }}</span>
                            </p>
            ...
            <div class="bg-white rounded-2xl border border-amber-200 shadow-sm p-5">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="text-sm font-semibold text-amber-700">En attente d'approbation</h3>
                    <a href="{{ route('booking.reservations.index') }}" class="text-xs text-amber-600 hover:text-amber-700 font-medium">Gérer →</a>
                </div>
                <div class="space-y-2">
