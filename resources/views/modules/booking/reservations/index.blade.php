    ...
    <div x-data="reservationsApp()" @keydown.escape.window="closeModal()">
        ...
        <div class="flex items-center justify-between mb-5">
            <div>
                ...
                <p class="text-sm text-gray-500">{{ $bookings->total() }} réservation(s)</p>
            </div>
            ...
            <table class="w-full text-sm">
                <thead class="bg-gray-50 border-b border-gray-200">
                    <tr>
                        ...
                        <th class="text-left px-4 py-3 font-semibold text-gray-600">Ressource</th>
                        <th class="text-left px-4 py-3 font-semibold text-gray-600">Demandeur</th>
                        ...
                        <th class="text-center px-4 py-3 font-semibold text-gray-600">Participants</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-3">
                            ...
                            @if($b->description)
                            <div class="text-xs text-gray-400 truncate max-w-xs">{{ $b->description }}</div>
                            @endif
                        </td>
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-2">
                                <span class="w-2.5 h-2.5 rounded-full shrink-0" style="background:{{ $b->resource->color ?? '#EA580C' }}"></span>
                                <span class="text-gray-700">{{ $b->resource->name }}</span>
                            </div>
                        </td>
                        <td class="px-4 py-3 text-gray-600">{{ $b->user->name }}</td>
                        <td class="px-4 py-3 text-center text-gray-700 whitespace-nowrap">{{ $b->start_at->format('d/m/Y H:i') }}</td>
                        <td class="px-4 py-3 text-center text-gray-700 whitespace-nowrap">{{ $b->end_at->format('d/m/Y H:i') }}</td>
                        <td class="px-4 py-3 text-center text-gray-700">{{ $b->attendees_count }}</td>
                        <td class="px-4 py-3 text-center">
                            <span class="px-2 py-0.5 rounded-full text-xs font-medium {{ $statusColors[$b->status] ?? 'bg-gray-100 text-gray-500' }}">
                        ...
                        <td class="px-4 py-3 text-right whitespace-nowrap">
                            ...
                            <button @click="approve({{ $b->id }})"
                                    class="text-xs text-emerald-600 hover:text-emerald-700 font-medium mr-2">Approuver</button>
                            ...
                            <button @click="cancel({{ $b->id }})"
                                    class="text-xs text-gray-500 hover:text-gray-700 font-medium mr-3">Annuler</button>
                            @endif
                    ...
                    @empty
                    <tr><td colspan="8" class="px-4 py-8 text-center text-gray-400">Aucune réservation.</td></tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div x-show="showRejectModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-sm z-10 p-6 space-y-4" @click.stop>
                <h3 class="text-base font-semibold text-gray-900">Rejeter la réservation</h3>
                <div>
                ...
                <div class="flex justify-end gap-3">
                    <button @click="showRejectModal=false" class="px-4 py-2 text-sm border border-gray-300 rounded-lg hover:bg-gray-50">Annuler</button>
                    <button @click="confirmReject()" :disabled="loading"
                            class="px-5 py-2 text-sm font-semibold text-white bg-red-600 hover:bg-red-700 rounded-lg disabled:opacity-60">
                        <span x-show="!loading">Confirmer</span><span x-show="loading">…</span>
                    </button>
                </div>
            </div>
        </div>
