<x-app-layout>
    ...
    <div x-data="crudTable({
        ...
        <div class="flex items-center justify-between mb-5">
            <div>
                <h2 class="text-lg font-bold text-gray-900 dark:text-white">Matières</h2>
                <p class="text-sm text-gray-500 dark:text-gray-400">{{ $subjects->total() }} matière(s)</p>
            </div>
            ...
            <table class="w-full text-sm">
                <thead class="bg-gray-50 dark:bg-gray-700 border-b border-gray-200 dark:border-gray-600">
                    <tr>
                        ...
                        <th class="text-center px-4 py-3 font-semibold text-gray-600 dark:text-gray-400">Coeff.</th>
                        <th class="text-center px-4 py-3 font-semibold text-gray-600 dark:text-gray-400">ECTS</th>
                        <th class="text-center px-4 py-3 font-semibold text-gray-600 dark:text-gray-400">Couleur</th>
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                        <td class="px-4 py-3 font-mono font-semibold text-emerald-700 dark:text-emerald-400">{{ $subject->code }}</td>
                        <td class="px-4 py-3 font-medium text-gray-900 dark:text-white">{{ $subject->name }}</td>
                        <td class="px-4 py-3 text-center text-gray-700 dark:text-gray-300">{{ $subject->coefficient }}</td>
                        <td class="px-4 py-3 text-center">
                            <span class="px-2 py-0.5 rounded-full text-xs font-medium bg-violet-50 dark:bg-violet-900/30 text-violet-700 dark:text-violet-300">{{ $subject->ects }} ECTS</span>
                        </td>
                        <td class="px-4 py-3 text-center">
                            <span class="inline-block w-6 h-6 rounded-md border border-gray-200 dark:border-gray-600 shadow-sm"
                                  style="background: {{ $subject->color ?? '#1E40AF' }}"></span>
                        </td>
                        <td class="px-4 py-3 text-gray-500 dark:text-gray-400 truncate max-w-xs">{{ $subject->description ?? '—' }}</td>
                        <td class="px-4 py-3 text-right">
                            <button @click="openEdit({{ $subject }})" class="text-xs text-blue-600 dark:text-blue-400 hover:text-blue-700 font-medium mr-3">Modifier</button>
                            <button @click="deleteItem({{ $subject->id }})" class="text-xs text-red-500 hover:text-red-600 font-medium">Supprimer</button>
                        </td>
                    </tr>
                    @empty
                    <tr><td colspan="7" class="px-4 py-8 text-center text-gray-400 dark:text-gray-500">Aucune matière enregistrée.</td></tr>
                    @endforelse
                </tbody>
            </table>
            @if($subjects->hasPages())
            <div class="px-4 py-3 border-t border-gray-100 dark:border-gray-700">{{ $subjects->links() }}</div>
            @endif
        </div>
        ...
        <div x-show="showModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4"
            ...
            <div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-md z-10 p-6 space-y-4" @click.stop>
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <input type="text" x-model="form.color" placeholder="#1E40AF"
                                   class="flex-1 px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg text-sm font-mono focus:outline-none focus:ring-2 focus:ring-blue-500">
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">ECTS *</label>
                ...
                <div>
                    ...
                    <textarea x-model="form.description" rows="2" placeholder="Description optionnelle…"
                              class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none"></textarea>
                </div>
