    ...
    <div x-data="crudTable({
        ...
        <div class="flex items-center justify-between mb-5">
            <div>
                ...
                <p class="text-sm text-gray-500">{{ $groups->total() }} groupe(s)</p>
            </div>
            ...
            <table class="w-full text-sm">
                <thead class="bg-gray-50 border-b border-gray-200">
                    <tr>
                        ...
                        <th class="text-left px-4 py-3 font-semibold text-gray-600">Niveau</th>
                        <th class="text-left px-4 py-3 font-semibold text-gray-600">Année académique</th>
                        <th class="text-center px-4 py-3 font-semibold text-gray-600">Capacité</th>
                        <th class="text-left px-4 py-3 font-semibold text-gray-600">Groupe parent</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-3 font-mono font-semibold text-amber-700">{{ $group->code }}</td>
                        <td class="px-4 py-3 font-medium text-gray-900">{{ $group->name }}</td>
                        <td class="px-4 py-3 text-gray-700">
                            {{ $group->level?->name ?? '—' }}
                            @if($group->level?->faculty)
                                <span class="text-gray-400">· {{ $group->level->faculty->name }}</span>
                            @endif
                        </td>
                        <td class="px-4 py-3 text-gray-600">{{ $group->academicYear?->name ?? '—' }}</td>
                        <td class="px-4 py-3 text-center text-gray-700">{{ $group->capacity }}</td>
                        <td class="px-4 py-3 text-gray-500">{{ $group->parent?->name ?? '—' }}</td>
                        <td class="px-4 py-3 text-right">
                            <button @click="openEdit({{ $group }})" class="text-xs text-blue-600 hover:text-blue-700 font-medium mr-3">Modifier</button>
                            <button @click="deleteItem({{ $group->id }})" class="text-xs text-red-500 hover:text-red-600 font-medium">Supprimer</button>
                        </td>
                    </tr>
                    @empty
                    <tr><td colspan="7" class="px-4 py-8 text-center text-gray-400">Aucun groupe enregistré.</td></tr>
                    @endforelse
                </tbody>
            </table>
            @if($groups->hasPages())
            ...
            <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-md z-10 p-6 space-y-4" @click.stop>
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <p x-show="errors.capacity" x-text="errors.capacity" class="mt-1 text-xs text-red-600"></p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Groupe parent</label>
