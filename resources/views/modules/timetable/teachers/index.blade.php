<x-app-layout>
    ...
    <div x-data="crudTable({
        ...
        <div class="flex items-center justify-between mb-5">
            <div>
                <h2 class="text-lg font-bold text-gray-900">Enseignants</h2>
                <p class="text-sm text-gray-500">{{ $teachers->total() }} enseignant(s)</p>
            </div>
            ...
            <table class="w-full text-sm">
                <thead class="bg-gray-50 border-b border-gray-200">
                    <tr>
                        ...
                        <th class="text-left px-4 py-3 font-semibold text-gray-600">Spécialité</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-3">
                            ...
                            <div class="text-xs text-gray-400">{{ $teacher->user->email }}</div>
                        </td>
                        <td class="px-4 py-3 font-mono font-semibold text-rose-700">{{ $teacher->employee_code }}</td>
                        <td class="px-4 py-3 text-gray-600">{{ $teacher->title ?? '—' }}</td>
                        <td class="px-4 py-3 text-gray-600">{{ $teacher->speciality ?? '—' }}</td>
                        <td class="px-4 py-3">
                            @php
                                $contractLabels = ['permanent'=>'Permanent','contractuel'=>'Contractuel','vacataire'=>'Vacataire'];
                                $contractColors = ['permanent'=>'bg-blue-50 text-blue-700','contractuel'=>'bg-amber-50 text-amber-700','vacataire'=>'bg-gray-100 text-gray-600'];
                        ...
                        <td class="px-4 py-3 text-right">
                            <button @click="openEdit({{ $teacher }})" class="text-xs text-blue-600 hover:text-blue-700 font-medium mr-3">Modifier</button>
                            <button @click="deleteItem({{ $teacher->id }})" class="text-xs text-red-500 hover:text-red-600 font-medium">Supprimer</button>
                        </td>
                    </tr>
                    @empty
                    <tr><td colspan="7" class="px-4 py-8 text-center text-gray-400">Aucun enseignant enregistré.</td></tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div x-show="showModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-md z-10 p-6 space-y-4" @click.stop>
                ...
                {{-- User selection only shown in create mode --}}
                <div x-show="!editMode">
