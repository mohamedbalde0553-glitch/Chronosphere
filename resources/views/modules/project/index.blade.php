<x-app-layout>
    ...
    <div x-data="projectsApp()">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-12 text-center">
            <p class="text-gray-400 dark:text-gray-500">Aucun projet. Créez-en un pour commencer.</p>
        </div>
        @else
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm hover:shadow-md transition-shadow p-5 flex flex-col gap-4">
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400 line-clamp-2 -mt-1">{{ $project->description }}</p>
                @endif
                ...
                <div class="flex items-center gap-2 pt-2 border-t border-gray-100 dark:border-gray-700">
                    <a href="{{ route('project.projects.board', $project) }}"
                       class="flex-1 text-center px-3 py-1.5 text-xs font-semibold text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-900/30 hover:bg-indigo-100 dark:hover:bg-indigo-900/50 rounded-lg transition-colors">
                        Kanban
                    </a>
                    <a href="{{ route('project.projects.gantt', $project) }}"
                       class="flex-1 text-center px-3 py-1.5 text-xs font-semibold text-violet-600 dark:text-violet-400 bg-violet-50 dark:bg-violet-900/30 hover:bg-violet-100 dark:hover:bg-violet-900/50 rounded-lg transition-colors">
                        Gantt
                    </a>
                    <button @click="openEdit({{ $project }})"
                            class="px-3 py-1.5 text-xs font-medium text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors">
                        Modifier
                    </button>
        ...
        <div x-show="showModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4"
            ...
            <div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-lg z-10 p-6 space-y-4" @click.stop>
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <input type="text" x-model="form.color"
                                   class="flex-1 px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg text-sm font-mono focus:outline-none focus:ring-2 focus:ring-indigo-500">
                        </div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea x-model="form.description" rows="2"
                              class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none"></textarea>
                </div>
                ...
                <div class="flex justify-end gap-3 pt-2">
                    ...
                    <button @click="save()" :disabled="loading"
                            class="px-5 py-2 text-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg disabled:opacity-60 inline-flex items-center gap-2">
            ...
            return {
                ...
                async save() {
                    ...
                    try {
                        ...
                        window.dispatchEvent(new CustomEvent('toast', {detail:{message:'Projet enregistré',type:'success'}}));
                ...
                async deleteProject(id) {
                    ...
                    try {
                        ...
                        if (!res.ok) throw new Error();
                        window.dispatchEvent(new CustomEvent('toast', {detail:{message:'Projet supprimé',type:'success'}}));
